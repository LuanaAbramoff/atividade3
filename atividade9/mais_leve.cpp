...
bool ordenaLeve (Objeto i, Objeto j){
    return (i.peso < j.peso);
}

pair<int, int> maisLeve (std::vector<Objeto>& itens, std::vector<Objeto>& mochila, int& W){

    int peso_total = 0;
    int valor_total = 0;

    sort(itens.begin(), itens.end(), ordenaLeve);
...
int main() {
    ...
    pair<int, int> resultado = maisLeve(itens, mochila, W);
