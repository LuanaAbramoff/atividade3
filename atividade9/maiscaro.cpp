...
bool ordenaMaisCaro (Objeto i, Objeto j){
    return (i.valor > j.valor);
}

pair<int, int> maisCaro (std::vector<Objeto>& itens, std::vector<Objeto>& mochila, int& W){

    int peso_total = 0;
    int valor_total = 0;

    sort(itens.begin(), itens.end(), ordenaMaisCaro);
...
int main() {
    ...
    lerArquivo("Entrada_5.txt", itens, W);

    //auto start = std::chrono::high_resolution_clock::now(); 

    pair<int, int> resultado = maisCaro(itens, mochila, W);
